...
def apply_target_encoding(df, encoder_pkl_path='models/target_encoder.pkl'):
    logger.debug('Target encoding...')
...
def run_preproc(input_df,
    ...
    logger.info('Imputation of continuous features done.')
    ...
    if missing:
        logger.warning('Missing columns in test data: %s. Filling with 0.', missing)
        for col in missing:
            df[col] = 0
    df = df[final_columns]

    logger.info('Preprocessing finished. Output shape: %s', df.shape)
    return df